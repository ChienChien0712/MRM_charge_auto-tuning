...
if 'methods' not in os.listdir(outputFile_url):
    os.mkdir('%s/methods_heavy'%outputFile_url)
...
formula_K = dict(zip(formula[formula['C-term']=='K']['Peptide'],formula[formula['C-term']=='K']['Formula']))
formula_R = dict(zip(formula[formula['C-term']=='R']['Peptide'],formula[formula['C-term']=='R']['Formula']))
...
for i in OutputFile.index:
    ...
    if OutputFile['Q1 peptide sequence'][i][-1] == 'K':
        Q1.append(eval(formula_K[OutputFile['Q1 charge'][i]]))
    elif OutputFile['Q1 peptide sequence'][i][-1] == 'R':
        Q1.append(eval(formula_R[OutputFile['Q1 charge'][i]]))
    ...
    if OutputFile['Q1 peptide sequence'][i][-1] == 'K':
        Q3.append(eval(formula_K[OutputFile['Q3 charge'][i]]))
    elif OutputFile['Q1 peptide sequence'][i][-1] == 'R':
        Q3.append(eval(formula_R[OutputFile['Q3 charge'][i]]))
OutputFile['Q1'] = Q1
OutputFile['Q3'] = Q3
...
for i in OutputFile.index:
    if (OutputFile['Q1 charge'][i]=='Q1+++')&(OutputFile['Q1'][i]>=DoPrecursorCharge4):
        ...
        if OutputFile.at[OutputFile.index[-1],'Q1 peptide sequence'][-1] == 'K':
            OutputFile.at[OutputFile.index[-1],'Q1'] = eval(formula_K[OutputFile.at[OutputFile.index[-1],'Q1 charge']])
        elif OutputFile.at[OutputFile.index[-1],'Q1 peptide sequence'][-1] == 'R':
            OutputFile.at[OutputFile.index[-1],'Q1'] = eval(formula_K[OutputFile.at[OutputFile.index[-1],'Q1 charge']])
...
for i in OutputFile.index:
    if OutputFile['product charge'][i] == '+1':
        Name.append('%s_%s.%s.+%s%s.heavy'%(OutputFile['uniprot'][i],OutputFile['species'][i],OutputFile['Q1 peptide sequence'][i],
                                           OutputFile['precursor charge'][i],OutputFile['split name'][i]))
    else:
        Name.append('%s_%s.%s.+%s%s%s.heavy'%(OutputFile['uniprot'][i],OutputFile['species'][i],OutputFile['Q1 peptide sequence'][i],
...
for sepcol in Sep_columns:
    if sum(pd.isna(MethodFile[sepcol])) != len(MethodFile[sepcol]):
        for j in [i for i in MethodFile[sepcol] if not pd.isna(i)]:
            ...
            if 'seperated' not in os.listdir('%s/methods_heavy'%outputFile_url):
                os.mkdir('%s/methods_heavy/seperated'%outputFile_url)
            if DefaultFile_url == 'None':
                OutputFile2[output_columns].to_csv('%s/methods_heavy/seperated/seperated_%s%d.csv'%(outputFile_url,sepcol.split('_')[0].upper(),j),index=False)
            else:
                OutputFile2[output_columns].to_csv('%s/methods_heavy/seperated/seperated_%s%d(best_default=true).csv'%(outputFile_url,sepcol.split('_')[0].upper(),j),index=False)
...
for rampcol in Ramping_columns:
    if sum(pd.isna(MethodFile[rampcol])) != len(MethodFile[rampcol]):
        ...
        if 'ramping' not in os.listdir('%s/methods_heavy'%outputFile_url):
            os.mkdir('%s/methods_heavy/ramping'%outputFile_url)
        if DefaultFile_url == 'None':
            OutputFile2[output_columns].to_csv('%s/methods_heavy/ramping/%s_ramping.csv'%(outputFile_url,rampcol.split('_')[0].upper()),index=False)
        else:
            OutputFile2[output_columns].to_csv('%s/methods_heavy/ramping/%s_ramping(best_default=true).csv'%(outputFile_url,rampcol.split('_')[0].upper()),index=False)