...
filename = [i for i in filename if ('DP' in i)|('EP' in i)]
filename = filename[0]
...
Name = list(f1['name'])
f1['name2'] = [i.split('.')[1]+'_'+i.split('.')[2][1] for i in Name]
Answer = dict(zip(f1['name2'],f1[CHARGE]))
...
if DoDistance == 'TRUE':
    for method in Methods:
        ...
        for fn in FileList:
            ...
            f2['%s%s'%(CHARGE,ramp_sep_type)] = f2['%s%s'%(CHARGE,ramp_sep_type)].astype('str')  
            ...
            #plt.xlim(min(answer_value+modeled_value)-CHARGE_SPAN,max(answer_value+modeled_value)+CHARGE_SPAN)
            #plt.ylim(min(answer_value+modeled_value)-CHARGE_SPAN,max(answer_value+modeled_value)+CHARGE_SPAN)
            plt.xticks(CHARGE_RANGE)
            plt.yticks(CHARGE_RANGE)
            plt.grid(linestyle='--')
            plt.plot(CHARGE_RANGE,CHARGE_RANGE,color='black')
            plt.legend()
            plt.savefig('%s/%s/%s/(Diff)%s.png'%(prediction_folder,filename,method,fn.split('.')[0]),dpi=100,bbox_inches='tight')
            plt.clf()
        ...
        for fn in FileList:
            ...
            for n in Modeled_name:
                if n.split(';')[0] in Answer:
                    answer_value.append(Answer[n.split(';')[0]])
                    modeled_value.append(Modeled[n])
            if len(set(modeled_value))==1:
                Method_list.append(fn.split('.')[0])
                PointNumber_list.append(len(answer_value))
                R_squared_list.append('NA')
                R_list.append('NA')
                MSE_residuals.append('NA')
                ...
                plt.xlabel('Estimated %s'%CHARGE,fontsize=14)
                plt.ylabel('Standard %s'%CHARGE,fontsize=14)
                plt.savefig('%s/%s/%s/(MSE)%s.png'%(prediction_folder,filename,method,fn.split('.')[0]),dpi=100,bbox_inches='tight')
                plt.clf()            
                continue
            ...
            plt.yticks(CHARGE_RANGE)
            plt.grid(linestyle='--')
            #plt.xlim(min(answer_value+modeled_value)-15,max(answer_value+modeled_value)+15)
            #plt.ylim(min(answer_value+modeled_value)-15,max(answer_value+modeled_value)+15)
            plt.title('%s\n'%filename + fn.split('.')[0],fontsize=16)
            plt.xlabel('Estimated %s'%CHARGE,fontsize=14)
            plt.ylabel('Standard %s'%CHARGE,fontsize=14)
            plt.legend()
