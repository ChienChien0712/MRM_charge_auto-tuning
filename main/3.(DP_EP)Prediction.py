...
f1['Name'] = f1[['Peptide Sequence','Precursor Charge']].apply(lambda x: '_'.join(x), axis = 1) 
...
for name in Name:
    ...
    Opt_Step = sorted(list(set(f2['Opt Step'])))
    y_Area = []
    for os2 in Opt_Step:
        y_Area.append(sum(f2[(f2['Opt Step']==os2)]['Area_Background']))   
    if 'des' in name:
        y_Area.reverse()    
    ...
    DPramping_forcsv.append(list(f2['Replicate Name'])[0])
...
def FitPolynomial(f1,points_number,pol_level,margineRm=False):
    ...
    for n in Name:
        ...
        y_axis = []
        for os2 in Opt_Step:
            y_axis.append(sum(f2[(f2['Opt Step']==os2)]['Area_Background'])) 
        if 'des' in n:
            y_axis.reverse()      
...
for method in Methods:
    ...
    for name in Name:
        ...
        y_AreaBackground = []
        for os2 in Opt_Step:
            y_AreaBackground.append(sum(f2[(f2['Opt Step']==os2)]['Area_Background']))           
        if 'des' in name:
...
for method in Methods:
    ...
    for name in Name:
        ...
        if plot_pic=='TRUE':
            ...
            if y_max_rmMargin != 0:
                plt.scatter(x_max_rmMargin,y_max_rmMargin,color='red')
                plt.annotate('DP=%.1f'%x_max_rmMargin, (x_max_rmMargin,y_max_rmMargin),color='red')            
